
Ricardo Jovito

Estudante de Análise e Desenvolvimento de Sistemas



📫 Contato & Redes Sociais

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge\&logo=linkedin\&logoColor=white)](https://github.com/sampabonyy-source)

[![E-mail](https://img.shields.io/badge/Email-D14836?style=for-the-badge\&logo=gmail\&logoColor=white)](sampabonyy@gmail.com)


👨‍💻 Sobre

Atualmente cursando Análise e Desenvolvimento de Sistemas. Tenho interesse em tecnologia aplicada, com foco em usabilidade, prototipagem e desenvolvimento de sistemas voltados à solução de problemas reais.

* **Habilidades Técnicas:** SQL, Modelagem de Dados, Figma, Prototipagem, UX/UI.
* **Habilidades Socioemocionais:** Comunicação, organização, pensamento crítico e trabalho em equipe.
* **Projetos Relevantes:** Desenvolvimento de sistema de orçamento para empresa de limpeza e auditoria de usabilidade baseada nas heurísticas de Nielsen.



🛠 Minhas Ferramentas Favoritas
IA
ChatGPT 
Claude 
Google Gemini 

💻 Tecnologias e Ferramentas

![Figma](https://img.shields.io/badge/Figma-000000?style=for-the-badge\&logo=figma\&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-336791?style=for-the-badge\&logo=postgresql\&logoColor=white)
![Balsamiq](https://img.shields.io/badge/Balsamiq-CC0000?style=for-the-badge\&logoColor=white)

🌐 Front-end
![HTML5](https://img.shields.io/badge/HTML5-Markup-orange?logo=html5)
![CSS3](https://img.shields.io/badge/CSS3-Style-blue?logo=css3)




📂 Portfólio de Projetos

#### 🤖 Inteligência Artificial

### 🤖 Projetos com Inteligência Artificial

| Projeto                        | Descrição                                                                                     |                         Link                        |
| :----------------------------- | :-------------------------------------------------------------------------------------------- | :-------------------------------------------------: |
| **SM1 - Corrida do Prompt**    | Exploração de técnicas de engenharia de prompt para otimização de respostas em IA generativa. |   https://github.com/sampabonyy-source/POTIFOLIO-RICARDO-JOVITO/tree/ccd8eb8dc13bf56280063211b01d2b8fb2d61e8e/A%20Corrida%20do%20Prompt   |
| **SM2 - Classificação Visual** | Desenvolvimento de modelo de classificação de imagens com foco em visão computacional.        | https://github.com/sampabonyy-source/POTIFOLIO-RICARDO-JOVITO/tree/87bfaa2697432d1ce989bb80f7d893708977a958/Batalha%20de%20Modelos%20%26%20Engenharia%20de%20Prompt%20(XML) |
| **SM3 - Batalha de Modelos**   | Comparação entre diferentes modelos de IA e uso de prompts estruturados em XML.               |    https://github.com/sampabonyy-source/POTIFOLIO-RICARDO-JOVITO/tree/423ccc42a402ecb7a073d6a154fadacc0ec3bd2c/Batalha%20de%20Modelos%20%26%20Engenharia%20de%20Prompt%20(XML) |
| **SM4 - Engenharia Reversa**   | Análise e evolução de sistemas baseados em IA para compreender seu funcionamento interno.     |  https://github.com/sampabonyy-source/POTIFOLIO-RICARDO-JOVITO/tree/92adc20937ff9ce40cdee2d7cec7e926134397c2/Engenharia%20Reversa |
| **SM5 - MVP com IA**           | Criação de um produto mínimo viável utilizando IA, do conceito à aplicação prática.           |        https://github.com/sampabonyy-source/POTIFOLIO-RICARDO-JOVITO/tree/44b5e8c7d7171d5ecd5e1085441d6beb7182c384/Do%20Clone%20ao%20Produto%20M%C3%ADnimo%20Vi%C3%A1vel%20(MVP%2B)    |
| **SM7 - IA com Bubble.io**     | Desenvolvimento de aplicação com IA integrada à plataforma no-code Bubble.io.                 |       [https://github.com/sampabonyy-source/POTIFOLIO-RICARDO-JOVITO/tree/5dfd9cbbbb8a33bbbb10255aed870a82df815d7b/Engenharia%20de%20Software%20e%20IA%20com%20Bubble.io    |

🏗 Estrutura do Repositório

A organização foi definida para facilitar navegação e entendimento:

* `README.md`: Apresentação principal
* `/projetos/`: Sistemas e aplicações desenvolvidas
* `/ux-ui/`: Projetos de usabilidade e prototipagem
* `/documentos/`: Currículo e certificados
* `/artigos/`: Trabalhos acadêmicos
* `/assets/`: Imagens e recursos visuais
* `/extras/` (opcional): Estudos, testes e ideias em desenvolvimento


Sobre este Repositório

Repositório criado para centralizar e organizar a produção acadêmica e técnica, funcionando como uma vitrine profissional. Reúne projetos e materiais de forma estruturada, destacando habilidades e evolução ao longo da formação.
